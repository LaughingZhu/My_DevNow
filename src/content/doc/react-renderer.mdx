...
- `BeforeMutationMask`由update和snapshot组成，代表 ”BeforeMutation“ 阶段与这两个 flags 有关（由于SnapShot 是 ”ClassComponent 存在更新，且定义了 getSnapshotBeforeUpdate 方法“后才会设置的 flags，因此可以判断 BeforeMutation 阶段会执行 getSnapshotBeforeUpdate 犯法）。
